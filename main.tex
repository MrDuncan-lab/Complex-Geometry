...
Теперь $X$ --- комплексное многообразие, $E\to X$ --- голоморфное векторное расслоение, а $\mathcal A^{q}(E)$ --- пучок $(0,q)$-форм со значениями в $E$. Рассмотрим комплекс пучков
\[
0 \to \mathcal{O}(E) \to A^{0}(E) \xrightarrow{\overline{\partial}} A^{1}(E) \xrightarrow{\overline{\partial}} A^{2}(E) \to \cdots
\]
По \hyperref[th:dbar-poincare]{лемме Пуанкаре} этот комплекс является точной последовательностью, то есть для каждого $q\ge1$ имеем короткие точные последовательности пучков
\[
...
Напомним общий факт: для любой короткой точной последовательности пучков
\[
0 \longrightarrow \mathcal F \longrightarrow \mathcal G \longrightarrow \mathcal H \longrightarrow 0
\]
существует ассоциированная длинная точная последовательность когомологий
\begin{multline}
...
Пучки $A^p(E)$ (а значит и $\mathcal A^p(E)$) являются тонкими, поэтому их высшие когомологии исчезают:
\[
H^q(X, \mathcal{F}^p(E)) = 0,\quad q \geq 1.
\]
Применяя длинную точную последовательность когомологий к короткой точной последовательности
\[
0 \to \mathcal{O}(E) \hookrightarrow A^{0}(E) \xrightarrow{\bar\partial} \bar\partial A^{0}(E) \to 0,
\]
 получаем для $q\ge2$ изоморфизмы
\[
H^q(X, \mathcal{O}(E)) \cong H^{q-1}(X, \overline{\partial} \mathcal{A}^0(E)), \quad q \geq 2.
\]

Для малых значений $q$ из той же длинной точной последовательности имеем 
\[
0 \leftarrow H^1(X, \partial(E)) \leftarrow H^0(X, \overline{\partial} \mathcal{A}^0(E)) 
\xleftarrow{\ \overline{\partial} \ } H^0(X, \mathcal{A}^0(E)) 
\leftarrow H^0(X, \partial(E)) \leftarrow 0.
\]
(Здесь $\partial(E)$ обозначает тот же пучок, что и $\mathcal O(E)$.) Отсюда немедленно следует
\[
H^1(X, \mathcal O(E)) \cong \frac{H^0(X, \overline{\partial} \mathcal{A}^0(E))}{\overline{\partial} H^0(X, \mathcal{A}^0(E))}.
\]

Аналогично, применяя длинную точную последовательность к последовательностям
\[
0 \to \bar\partial A^{q-1}(E) \hookrightarrow A^{q}(E) \xrightarrow{\bar\partial} \bar\partial A^{q}(E) \to 0 ,
\]
и снова используя исчезновение когомологий тонких пучков, комбинируя полученные изоморфизмы с изложенным выше случаем, получаем, что для всех $q\ge1$
\[
...
\]
С другой стороны, по определению когомологий Дольбо
\[
...
\]
Так как $H^0(X, \overline{\partial} \mathcal{A}^{q-1}(E))$ состоит из $\overline{\partial}$-замкнутых $(0,q)$-форм со значениями в $E$, а 
$\overline{\partial} H^0(X, \mathcal{A}^{q-1}(E))$ — из $\overline{\partial}$-точных форм, получаем 
\[
H^q(X,\partial(E)) \cong H_{\overline{\partial}}^q(X,E).
\]
